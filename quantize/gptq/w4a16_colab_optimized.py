# %%
"""
W4A16 GPTQ 양자화 - 전체 파라미터 최적화 + 서버 호환 버전
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
파라미터가이드.md 기반 모든 최적화 + group_size 직접 설정 + version 패치
...
적용된 최적화:
  ...
  🔴 트레이드오프 (정확도↑ 속도↓):
    - group_size: 128 → 64 (config_groups 직접 설정)
    - block_size: 128 → 64

  🔧 서버 호환:
    - compressed-tensors version → 0.13.0 패치
    - transformers → 4.57.3
...
try:
    ...
    mlflow.set_experiment("W4A16-all-params-v2")
    USE_MLFLOW = True
except:
    USE_MLFLOW = False
...
# ── 양자화 비트 설정 (config_groups 직접 설정) ──
NUM_BITS = 4                          # 가중치 비트 수
GROUP_SIZE = 64                       # ⭐ 128→64 (scale 공유 그룹 크기, 정확도 ↑)

# ── 🟢 공짜 정확도 (추론 속도 영향 0) ──
ACTORDER = "dynamic"                  # ⭐ static→dynamic (공짜 정확도 ↑↑↑)
DAMPENING_FRAC = 0.01                 # 양자화 안정성
NUM_CALIBRATION_SAMPLES = 256        # ⭐ 256→1024 (공짜 정확도 ↑↑↑)
MAX_SEQUENCE_LENGTH = 512             # ⭐ 512→768 (공짜 정확도 ↑↑)
SEQUENTIAL_TARGETS = ["Exaone4DecoderLayer"]  # ⭐ 레이어별 순차 처리 (공짜 정확도 ↑)

# ── 🔴 트레이드오프 (정확도↑ 속도 약간↓) ──
BLOCK_SIZE = 64                       # ⭐ GPTQ 열 처리 단위 (정확도 ↑)

# ── 🔵 공짜 속도 (정확도 영향 0) ──
MAX_POSITION_EMBEDDINGS = 32768       # ⭐ 65536→32768 (KV Cache 50% 절감!)

# ── 🔧 서버 호환 ──
TARGET_CT_VERSION = "0.13.0"          # 평가 서버의 compressed-tensors 버전
...
├──────────────────────────────────────────────┤
│ 🔴 트레이드오프 (정확도↑ 속도↓)                │
│   group_size={GROUP_SIZE}  (config_groups)   │
│   block_size={BLOCK_SIZE}  (GPTQ 처리단위)   │
│   ignore={IGNORE}                            │
├──────────────────────────────────────────────┤
│ 🔵 공짜 속도 (정확도 영향 0)                   │
...
print(f"\n[2/8] 캘리브레이션 데이터 {NUM_CALIBRATION_SAMPLES}개 로드")
...
print(f"\n[3/8] GPTQ 양자화 시작 (전체 파라미터 최적화 + group_size={GROUP_SIZE})")
...
print(f"  🔴 group_size={GROUP_SIZE}, block_size={BLOCK_SIZE}")
print(f"  🔴 ignore={IGNORE}")

# config_groups로 직접 설정하여 group_size 반영
recipe = [
    GPTQModifier(
        ...
        config_groups={
            "group_0": QuantizationScheme(
                ...
                weights=QuantizationArgs(
                    num_bits=NUM_BITS,
                    type="int",
                    symmetric=True,             # 기본값 유지 (observer/symmetric 변경 안 함)
                    strategy="group",
                    group_size=GROUP_SIZE,       # ⭐ 여기서 group_size 직접 설정!
                ),
            )
        },
    )
...
if "quantization_config" in config:
    ...
    # group_size 확인
    cg = config["quantization_config"].get("config_groups", {})
    for group_name, group_cfg in cg.items():
        if isinstance(group_cfg, dict) and "weights" in group_cfg:
            gs = group_cfg["weights"].get("group_size", "N/A")
            print(f"  → {group_name} group_size: {gs}")
...
# %% [셀 10] ZIP 생성
print(f"\n[6/8] ZIP 생성")
zip_name = "w4a16_all_params_v2"
shutil.make_archive(f"/content/{zip_name}", "zip", "/content", "model")
...
try:
    ...
    print(f"  → Colab 환경이 아닙니다. 수동 다운로드: {zip_path}")
# -------------------------------------------------------------------
# %% [셀 11] MLflow 기록 (선택)
print(f"\n[7/8] MLflow 기록")
if USE_MLFLOW:
    with mlflow.start_run(run_name=f"W4A16-gs{GROUP_SIZE}-bs{BLOCK_SIZE}-cal{NUM_CALIBRATION_SAMPLES}"):
        mlflow.log_params({
            "model_id": MODEL_ID,
            "scheme": f"W{NUM_BITS}A16",
            "actorder": ACTORDER,
...
# %% [셀 12] 완료 요약
print("\n" + "=" * 60)
print("✅ W4A16 전체 파라미터 최적화 + 서버 호환 양자화 완료!")
print("=" * 60)
print(f"""
📊 적용된 최적화 (vs 기존 0.57점 코드):
 ┌─ 🟢 공짜 정확도 (추론 속도 영향 0) ──────────┐
 ...
 │  shuffle_calibration_samples: True ⭐       │
 ├─ 🔴 트레이드오프 (정확도↑ 속도↓) ────────────┤
 │  group_size: 128 → {GROUP_SIZE} ⭐ (config_groups) │
