...
적용된 최적화:
  🟢 공짜 정확도 (추론 속도 영향 0):
    ...
    - shuffle_calibration_samples: True (신규)
    - sequential_targets: ["Exaone4DecoderLayer"] (신규)
  🔵 공짜 속도 (정확도 영향 0):
    - max_position_embeddings: 65536 → 32768 (신규)
  🔴 트레이드오프 (정확도↑ 속도↓):
    - block_size: 128 → 64 (신규)
    - ignore: +layers.0, +layers.29 (확장)
...
try:
    ...
    mlflow.set_experiment("W4A16-all-params")
    USE_MLFLOW = True
except:
    USE_MLFLOW = False
...
# ── 양자화 설정 ──
SCHEME = "W4A16"                      # 4-bit weight, 16-bit activation (유지)
TARGETS = ["Linear"]
IGNORE = [
...
MAX_SEQUENCE_LENGTH = 768            # ⭐ 512→1024 (공짜 정확도 ↑↑)
...
recipe = [
    GPTQModifier(
        ...
        dampening_frac=DAMPENING_FRAC,
        block_size=BLOCK_SIZE,
        sequential_targets=SEQUENTIAL_TARGETS,   # ⭐ 레이어별 순차 처리 (공짜 정확도 ↑)
    )
]
...
print(f"\n[7/7] MLflow 기록")
if USE_MLFLOW:
    with mlflow.start_run(run_name=f"W4A16-all-bs{BLOCK_SIZE}-cal{NUM_CALIBRATION_SAMPLES}"):
        mlflow.log_params({
            "model_id": MODEL_ID,
            "scheme": SCHEME,
            "actorder": ACTORDER,
...
# %% [셀 12] 완료 요약
print("\n" + "=" * 60)
print("✅ W4A16 전체 파라미터 최적화 양자화 완료!")
print("=" * 60)
print(f"""
📊 적용된 최적화 (vs 기존 0.57점 코드):
 ...
 │  sequential_targets: 미사용 → {SEQUENTIAL_TARGETS} ⭐ │
 │  shuffle_calibration_samples: True ⭐       │
 ├─ 🔴 트레이드오프 (정확도↑ 속도↓) ────────────┤
 │  block_size: 128 → {BLOCK_SIZE} ⭐              │
 │  ignore: +layers.0, +layers.29 ⭐           │
 ├─ 🔵 공짜 속도 (정확도 영향 0) ───────────────┤
 │  max_position_embeddings: 65536 → {MAX_POSITION_EMBEDDINGS} ⭐ │
 └──────────────────────────────────────────────┘

🎯 전략: 파라미터가이드.md의 모든 최적화 적용
