...
$$PerfNorm_{model} = \frac{Perf_{model}}{Perf_{base\_model}}$$

- 기본 모델(EXAONE-4.0-1.2B) 대비 **토큰당 추론 시간 감소 비율**

$$SpeedNorm_{model} = 1 - \frac{\left(\dfrac{Time_{model}}{Tokens_{model}}\right)}{\left(\dfrac{Time_{base\_model}}{Tokens_{base\_model}}\right)}$$
